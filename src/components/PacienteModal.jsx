...
export default function PacienteModal() {
    ...
    const nombres_paciente = createRef();
    ...
    const handleEnviarPaciente = e => {
        e.preventDefault();
        const datos = {
            nombres_paciente: nombres_paciente.current.value,
        ...
        if (datosActual.id_paciente != null) {
            handleEditarDatos(datosActual.id_paciente, datos, 'paciente');
        } else {
            handleIngresarDatos(datos, 'paciente');
            toast.info(`Paciente ${datos.nombres_paciente + " " + datos.apellidos_paciente} creado correctamente`);
        }
    }
    return (
        <div className="p-4 ">
            ...
            <h2 className="text-center mb-4 text-xl font-bold">{datosActual.id_paciente ? 'Actualizar Paciente' : 'Crear Paciente'}</h2>
            <form noValidate onSubmit={handleEnviarPaciente} className="bg-white p-4 rounded grid grid-cols-2 gap-4">
                ...
                <div className={`${datosActual.id_paciente ? 'hidden' : ''}`}>
                ...
                <div>
                    ...
                    <input defaultValue={datosActual ? datosActual.nombres_paciente : ''} ref={nombres_paciente} type="text" name="nombres_paciente" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
                </div>
                <div>
                ...
                <div
                    className={`${datosActual.id_paciente ? 'hidden' : ''}`}
                >
                ...
                <div className="col-span-2 flex justify-end">
                    <button type="submit" className="bg-slate-800 text-white px-6 py-2 rounded-full hover:bg-slate-900 focus:outline-none focus:bg-slate-900">
                        {datosActual.id_paciente ? 'Actualizar' : 'Crear'}
                    </button>
                </div>
            </form>
        </div>
